/**
 * Blog List Page Core Runtime Script (SSR Mode)
...
(function() {
  ...
  const SELECTORS = {
    container: '[data-component-type="blog-list-page"]',
    categoryLink: '.blp-category-link',
    categoryToggle: '.blp-category-toggle',
    sortSelect: '.blp-sort-select',
    sidebarToggle: '.blp-sidebar-toggle',
    sidebar: '.blp-sidebar'
  };

  /**
   * Initialize all blog list page containers
   */
  function initBlogListPages() {
    const containers = document.querySelectorAll(SELECTORS.container);

    containers.forEach(container => {
      // Prevent duplicate initialization
      if (container.dataset.blpInitialized === 'true') return;
      container.dataset.blpInitialized = 'true';

      new BlogListPage(container);
    });
  }

  /**
   * BlogListPage Class (Simplified for SSR)
   */
  class BlogListPage {
    ...
    toggleCategory(toggleButton) {
      const categoryItem = toggleButton.closest('.blp-category-item');
      if (!categoryItem) {
        console.warn('[BlogListPage] Category item not found');
        return;
      }
      ...
      // Debug log
      console.log('[BlogListPage] Category toggled:', {
    ...
    toggleSidebar() {
      ...
      if (sidebar) {
        sidebar.classList.toggle('blp-sidebar-open');
      }
    }
    ...
    setSortSelectValue() {
      ...
      const currentSort = sortParam || dataSort || this.config.defaultSort || 'date-desc';
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initBlogListPages);
  } else {
    initBlogListPages();
  }
